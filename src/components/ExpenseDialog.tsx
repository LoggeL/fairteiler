      ...
      <DialogContent className="sm:max-w-[500px] rounded-2xl border-none shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-2">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input id="betrag" value={betrag} onChange={e => setBetrag(e.target.value)} placeholder="0,00" className="pr-12 font-bold text-lg h-12 bg-slate-50 border-none" type="number" step="0.01" />
            ...
            <div className="space-y-2">
              ...
              <select id="zahler" className="flex h-11 w-full rounded-md border-none bg-slate-50 px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" value={zahlerId} onChange={e => setZahlerId(e.target.value)}>
                  ...
                  <div className="space-y-2 bg-slate-50 p-3 rounded-xl">
                    {mitglieder.map(m => (
                      <div key={m.id} className="flex items-center gap-2">
                        ...
                        <Input id={`share-${m.id}`} value={shares.get(m.id) || ''} onChange={e => handleShareChange(m.id, e.target.value)} placeholder="0" type="number" className="flex-1 h-9 bg-white border-slate-200" />
                      </div>
                    ))}
                  </div>
                </TabsContent>
          ...
          <DialogFooter className="flex gap-2 sm:justify-between pt-4">
             {expense && <Button type="button" variant="ghost" className="text-slate-400 hover:text-red-500 hover:bg-red-50 h-11 px-4 rounded-xl" onClick={handleDelete} disabled={isDeleting}><Trash2 className="h-4 w-4" /></Button>}
